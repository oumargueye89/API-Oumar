...
"""
Pour chacune des variables dans 'main_cols', tracez une boîte à moustaches de l'ensemble des données (X_all),
puis un swarmplot des 20 valeurs variables des plus proches voisins (X_neigh),
et les valeurs du client demandeur (X_cust) en tant que pd.Series.
"""
...
def plot_boxplot_var_by_target(X_all, y_all, X_neigh, y_neigh, X_cust, main_cols, figsize=(15, 4)):
    ...
    # échantillon aléatoire de clients de la dataset
...
def plot_scatter_projection(X, ser_clust, n_display, plot_highlight, X_cust, proj="PCA",
    ...
    if len(columns) == 2:
        # si seulement 2 colonnes sont passées
        ...
        # si plus de 2 colonnes sont passées, calcule la projection
        if proj=="t-SNE":
    ...
    # Affichage des points, cluster par cluster
    colors = ['green', 'red']
    for i, name_clust in enumerate(ser_clust.unique()):
        ...
        # tracer uniquement une sélection aléatoire de points d'échantillonnage aléatoires
        ...
        # tracer les voisins les plus proches
